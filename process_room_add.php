...
$sql = "INSERT INTO `tb_room` (`RM_RoomName`,`FL_FloorID`,`DM_DepartmentID`,`RM_RoomSex`,`ST_StaffID`) VALUES(?,?,?,?,?)";
if ($stmt = $mysqli_asset->prepare($sql)) {
    $stmt->bind_param('ssiss', $RM_RoomName, $FL_FloorID, $DM_DepartmentID, $RM_RoomSex, $ST_StaffID);
    $stmt->execute();
